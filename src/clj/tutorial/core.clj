...
(defn multi-operator [coll]
  (loop [lst coll 
         idx 0]
        (if (or (= (nth lst idx) '*) (= (nth lst idx) '/))
          idx
          (recur lst 
                 (+ idx 1)))))

(defn add-operator [coll]
  (loop [lst coll 
         idx 0]
        (if (or (= (nth lst idx) '+) (= (nth lst idx) '-))
          idx
...
(defn making-multi [coll]
  (let [idx (multi-operator coll)
        infixed (infix (take 3 (drop (- idx 1) coll)))]
    (concat (take (dec idx) coll) (list infixed) (drop (+ idx 2) coll))))

(defn making-add [coll]
  (let [idx (add-operator coll)
        infixed (infix (take 3 (drop (- idx 1) coll)))]
    (concat (take (dec idx) coll) (list infixed) (drop (+ idx 2) coll))))

(defn making-eqn [coll]
  (loop [eqn coll]
        (cond 
          (= (list (first eqn)) eqn)
          (first eqn)
          (or (.contains eqn '*) (.contains eqn '/))
          (recur (making-multi eqn))
          (or (.contains eqn '+) (.contains eqn '-))
          (recur (making-add eqn)))))
...
(defmacro defattrs
  ([retriever attr] 
   `(def ~retriever (comp ~attr :attributes)))
  ([retriever attr & other]
   `(do (defattrs ~retriever ~attr) (defattrs ~@other))))
